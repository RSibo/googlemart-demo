...
export class MediaManager {
  ...
  private videoInputEl: HTMLVideoElement | null = null;
  ...
  public async playAudioChunk(base64Data: string) {
    try {
      ...
      if (this.playbackAudioContext.state === 'suspended') {
        ...
        this._log('Resumed playbackAudioContext in playAudioChunk');
      }
  ...
  public startVideoStreaming(stream: MediaStream) {
    ...
    this.videoInputEl = document.createElement('video');
    this.videoInputEl.srcObject = stream;
    this.videoInputEl.play();
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    this.videoInputInterval = setInterval(() => {
      if (this.videoInputEl && this.videoInputEl.videoWidth > 0 && this.videoInputEl.videoHeight > 0 && ctx) {
        canvas.width = Math.min(this.videoInputEl.videoWidth, 768);
        canvas.height = (canvas.width * this.videoInputEl.videoHeight) / this.videoInputEl.videoWidth;
        ctx.drawImage(this.videoInputEl, 0, 0, canvas.width, canvas.height);
  ...
  public stopVideoStreaming() {
    ...
    this.videoInputEl = null;
    this._log('Video streaming stopped');
  }

  public captureFrame(maxWidth?: number): string | null {
    if (!this.videoInputEl || this.videoInputEl.videoWidth === 0 || this.videoInputEl.videoHeight === 0) {
      return null;
    }
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    ...
    let width = this.videoInputEl.videoWidth;
    let height = this.videoInputEl.videoHeight;

    if (maxWidth && width > maxWidth) {
      width = maxWidth;
      height = (width * this.videoInputEl.videoHeight) / this.videoInputEl.videoWidth;
    }

    canvas.width = width;
    canvas.height = height;
    ctx.drawImage(this.videoInputEl, 0, 0, width, height);
    return canvas.toDataURL('image/jpeg', 0.8).split(',')[1]; // Higher quality for snapshot
  }

  public get isStreaming(): boolean {
    return this.videoInputStream !== null;
  }
